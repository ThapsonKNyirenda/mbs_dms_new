...
<form id="myForm" action="#" enctype="multipart/form-data" method="POST" onsubmit="return validateForm();">
    ...
    <?php
        $id=$_GET['file_id'];
        $sql2 = "SELECT * FROM $department WHERE id= '$id'";
    ...
    <div class="row mb-3">
                <label for="inputText" class="col-sm-2 col-form-label">Document Title</label>
                <div class="col-sm-6">
                <input type="text" class="form-control" placeholder="Doc. Title" name="title" required value=<?php echo $title;?>>
                </div>
            </div>
...
if(isset($_POST['users'])) {
    ...
    $stmt = $conn->prepare("UPDATE $department SET `title` = ?, `selected_users` = ? WHERE `id` = ?");
